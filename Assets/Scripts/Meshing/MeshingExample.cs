...
public class MeshingExample : MonoBehaviour
{
    ...
    [SerializeField] private Text sliderValueDisplay;
    ...
    private float originalClipDist;
    ...
    IEnumerator Start()
    {
        ...
        originalClipDist = Camera.main.nearClipPlane;
        meshManager.enabled = false;
        pointCloudManager.enabled = false;

        distanceSlider.minValue = magicLeapFeature.MinNearZ * 100;
        distanceSlider.value = distanceSlider.minValue;
    ...
    private void Update()
    {
        ...
        statusText.Clear();
        statusText.AppendLine($"Near Clipping Enforcement Mode: {magicLeapFeature.NearClipPolicy}");
        statusText.AppendLine();
        statusText.AppendLine("Current Meshing Settings:");
    ...
    {
        float cameraNearClipPlane = Camera.main.nearClipPlane * 100;
        if (distanceSlider.value < cameraNearClipPlane || distanceSlider.value > cameraNearClipPlane)
        {
            distanceSlider.SetValueWithoutNotify(cameraNearClipPlane);
            sliderValueDisplay.text = $"{distanceSlider.value}cm";
        }
    }
    ...
    {
        if (Camera.main != null)
            Camera.main.nearClipPlane = originalClipDist;
    }
    
    private void OnApplicationPause(bool pause)
    {
        if (!pause)
            magicLeapFeature.ApplyToCamera(Camera.main);
    }
    ...
    public void SetCameraClipPlane(float value)
    {
        Camera.main.nearClipPlane = value / 100;
        sliderValueDisplay.text = $"{value}cm";
    }
}
